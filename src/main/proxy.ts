...
import { HybridCompressor } from "./hybridCompressor.js";
...
app.post("/v1/chat/completions", async (req, res) => {
  ...
  if (ACTIVE && Array.isArray(payload.messages)) {
    payload.messages = await Promise.all(payload.messages.map(async (m: any) => {
      if (m.role === "user") {
        const content = typeof m.content === "string" ? m.content : JSON.stringify(m.content);
        const result = await hybridCompressor.compressPrompt(content);
        beforeTokens += result.stats.beforeTokens;
        afterTokens += result.stats.afterTokens;
...
// Status endpoint for checking model availability
app.get("/status", async (req, res) => {
  const modelStatus = hybridCompressor.getModelStatus();
  res.json({
    active: ACTIVE,
...
// Refresh model availability endpoint
app.post("/refresh-model", async (req, res) => {
  await hybridCompressor.refreshModelAvailability();
  const modelStatus = hybridCompressor.getModelStatus();
  res.json({ refreshed: true, models: modelStatus });
});
